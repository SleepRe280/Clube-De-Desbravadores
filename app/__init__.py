...
def create_app(config_class=Config):
    ...
    def inject_active_club_theme():
        ...
        from app.access import club_leadership_nav_visibility, get_admin_panel_permissions
        from app.models import Club

        admin_panel_ctx = {"admin_panel": get_admin_panel_permissions()}
        club_nav_ctx = {"club_nav": club_leadership_nav_visibility()}
    ...
    def inject_admin_nav_kw():
        ...
        if not getattr(current_user, "is_authenticated", False) or not is_super_admin():
            return {"admin_nav_kw": {}}
        ep = request.endpoint or ""
        if not ep.startswith("admin."):
            return {"admin_nav_kw": {}}
        cid = (request.args.get("clube_id") or "").strip()
        return {"admin_nav_kw": {"clube_id": cid}} if cid else {"admin_nav_kw": {}}
